Page({
  ...
  loadOrders() {
    ...
    app.request({
      ...
      this.setData({
        ...
        ordersCompleted: formatted.filter(o => o.status === 'completed' || o.status === 'refunded' || o.status === 'refund_pending'),
      });
  ...
  updateTabCounts(allOrders) {
    ...
    allOrders.forEach(o => {
      if (o.status === 'refunded' || o.status === 'refund_pending') counts.completed++;
