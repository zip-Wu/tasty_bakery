...
| **接单开关** | 一键停接单，可顺手填关店说明和恢复时间，顾客端立刻显示并拦住新下单（不影响已付款订单） |
| **营业时间** | 商家自己改营业时段，门店列表/首页/商品详情跟着变。只是展示文案，非营业时间照样能下单 |
...
**预扣 SQL 误下架商品（2026-08-06）**

库存 10 的商品买 8 个，支付后库存正确扣到 2，商品却被自动下架了。

原因：MySQL 单表 UPDATE 的 SET 子句从左到右求值。`SET stock = stock - 8, is_available = CASE WHEN stock - 8 <= 0 THEN 0 ...` 执行到 is_available 时 stock 已经是扣完的 2，`2 - 8 <= 0` 成立，就把商品置成下架。只要买的数量超过库存一半就会触发。

处理：系统不再自动改 is_available，上下架完全由商家在后台手动控制。售罄只由前端按库存为 0 显示。

**售罄商品卡死购物车（2026-08-06）**

顾客 A 买走最后库存后，顾客 B 的购物车里还留着这件商品，点支付提示失败，回点单页发现商品已售罄，但没有减号入口删不掉，购物车永远下不了单。

原因：售罄商品的减号按钮被隐藏（原来只在有库存时显示），同时失败返回没有告诉前端是哪件商品不足，购物车不知道该删哪件。

处理：售罄商品在购物车中保留减号可手动删除；后端在售罄/库存不足时返回商品 ID，前端自动把它从购物车移除，其余商品保留。
...
│   │   ├── admin.js              # 管理 API（订单/商品/录单/营收/制作清单/门店开关/营业时间）
